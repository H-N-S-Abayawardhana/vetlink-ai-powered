...
export const TOXIC_FOODS = [
  'chocolate', 'grapes', 'raisins', 'onions', 'garlic', 'xylitol', 'avocado', 'macadamia nuts', 'alcohol', 'caffeine'
];
...
export function generateDietPlan(input: DietPlanInput) : DietPlan {
  ...
  // Adjust by BCS
  if (input.bcs == null) {
    notes.push('BCS missing — use the BCS Calculator to assess body condition.');
  } else if (input.bcs <= 3) {
    // underweight - increase calories
    dailyCalories = Math.round(dailyCalories * 1.2);
    notes.push('Underweight: high-calorie, nutrient-dense diet; more frequent feeding; focus on protein.');
  } else if (input.bcs <= 5) {
    // ideal
    ...
    notes.push('Overweight: controlled calories, reduced fat, increased fiber, increased activity.');
  } else {
    // obese
    dailyCalories = Math.round(dailyCalories * 0.75);
    notes.push('Obese: strict calorie-deficit plan with high-fiber weight-management foods.');
  }
  ...
  if (lifeStage === 'puppy') {
    ...
    notes.push('Senior: consider lower calorie intake and joint-support nutrients.');
  }
  ...
  const cupsPerDay = Math.max(0.1, Math.round((dailyCalories / kcalPerCup) * 10) / 10);
  ...
  const treatAllowanceKcal = Math.round(dailyCalories * 0.10);

  // Exercise guidance
  let exerciseMinutes = 30;
  switch (input.activityLevel) {
    case 'couch_potato': exerciseMinutes = 20; break;
    case 'normal': exerciseMinutes = 30; break;
    case 'active': exerciseMinutes = 60; break;
    case 'working': exerciseMinutes = 90; break;
    default: exerciseMinutes = 30; break;
  }
  ...
  if (input.bcs != null && input.bcs <= 3) recommendedFoodTypes.push('High-calorie, nutrient-dense (growth/puppy style)');
  if (input.bcs != null && input.bcs >= 6) recommendedFoodTypes.push('High-fiber weight management diet');
  recommendedFoodTypes.push('Dry kibble (measure by cup)', 'Wet food (as supplement)', 'Combination feeding');
