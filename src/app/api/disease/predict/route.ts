...
export async function POST(request: NextRequest) {
  try {
    ...
    if (result.disease_probabilities) {
      ...
      disease_probabilities = {
        "Hip Dysplasia":
          result.predicted_disease === "Hip Dysplasia"
            ? primaryProb
            : remainingProb,
        Osteoarthritis:
          result.predicted_disease === "Osteoarthritis"
            ? primaryProb
            : remainingProb,
        IVDD: result.predicted_disease === "IVDD" ? primaryProb : remainingProb,
        Normal:
          result.predicted_disease === "Normal" ? primaryProb : remainingProb,
        "Patellar Luxation":
          result.predicted_disease === "Patellar Luxation"
            ? primaryProb
            : remainingProb,
      };
    }
